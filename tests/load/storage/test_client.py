...
def test_read_polars_csv(client: LoadStorageClient) -> None:
    expected_df = pl.DataFrame({"a": [1, 2]})

    with (
        patch.object(
            client, "get_path", return_value="s3://landing/test_source/f.csv"
        ),
        patch("lubrikit.load.storage.client.pl.read_csv", return_value=expected_df),
    ):
...
def test_read_polars_parquet(client: LoadStorageClient) -> None:
    ...
    with (
        ...
        patch(
            "lubrikit.load.storage.client.pl.read_parquet", return_value=expected_df
        ),
    ):
...
def test_read_polars_json(client: LoadStorageClient) -> None:
    ...
    with (
        ...
        patch.object(client, "s3", mock_s3),
        patch("lubrikit.load.storage.client.pl.read_json", return_value=expected_df),
    ):
...
def test_read_polars_excel(client: LoadStorageClient) -> None:
    ...
    with (
        ...
        patch.object(client, "s3", mock_s3),
        patch("lubrikit.load.storage.client.pl.read_excel", return_value=expected_df),
    ):
